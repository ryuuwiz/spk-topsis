<x-layouts.app :title="__('Edit Rating')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
            <form method="POST" action="{{ route('rating.update', $rating) }}">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select id="id_alternatif" name="id_alternatif" required
                        ...
                        @foreach($alternatif as $alternatif)
                            <option value="{{ $alternatif->id }}" {{ old('id_alternatif', $rating->id_alternatif) == $alternatif->id ? 'selected' : '' }}>
                                {{ $alternatif->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select id="id_kriteria" name="id_kriteria" required
                        ...
                        @foreach($kriteria as $kriteria)
                            <option value="{{ $kriteria->id }}" {{ old('id_kriteria', $rating->id_kriteria) == $kriteria->id ? 'selected' : '' }}>
                ...
                <div class="mb-6">
                    ...
                    <input id="nilai" type="number" name="nilai" value="{{ old('nilai', $rating->nilai) }}" required step="0.01" min="0"
                ...
                <div class="mb-6 p-4 bg-gray-50 dark:bg-zinc-700 rounded-lg">
                    <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Rating Information</h3>
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="font-medium text-gray-600 dark:text-gray-400">ID:</span>
                            <span class="text-gray-900 dark:text-white">{{ $rating->id }}</span>
                        </div>
                        <div>
                            <span class="font-medium text-gray-600 dark:text-gray-400">Created:</span>
                            <span class="text-gray-900 dark:text-white">{{ $rating->created_at->format('M d, Y H:i') }}</span>
                        </div>
                        <div>
                            <span class="font-medium text-gray-600 dark:text-gray-400">Last Updated:</span>
                            <span class="text-gray-900 dark:text-white">{{ $rating->updated_at->format('M d, Y H:i') }}</span>
                        </div>
                    </div>
                </div>
