<x-layouts.app :title="__('Batch Add Ratings')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
                {{ __('Batch Add Ratings') }}
            </h2>
        ...
        <!-- Batch Create Rating Form -->
        <div class="bg-white dark:bg-zinc-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
            <form method="POST" action="{{ route('rating.storeBatch') }}">
                @csrf
                ...
                <!-- Kriteria Ratings -->
                <div class="mb-6">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-3">{{ __('Ratings for Each Criteria') }}</h3>
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                        {{ __('Enter values for each criteria. If a rating already exists, it will be updated.') }}
                    </p>

                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        @foreach($kriteria as $kriteria)
                            <div class="bg-gray-50 dark:bg-zinc-700 p-4 rounded-lg">
                                <label for="nilai_{{ $kriteria->id }}" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                    {{ $kriteria->nama }} ({{ ucfirst($kriteria->atribut) }})
                                </label>
                                <input id="nilai_{{ $kriteria->id }}"
                                       type="number"
                                       name="nilai[{{ $kriteria->id }}]"
                                       value="{{ old('nilai.' . $kriteria->id) }}"
                                       step="0.01"
                                       min="0"
                                       class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-zinc-700 dark:text-white @error('nilai.' . $kriteria->id) border-red-500 @enderror">
