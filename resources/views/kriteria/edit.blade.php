        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
            <form method="POST" action="{{ route('kriteria.update', $kriterium) }}">
                ...
                <div class="mb-4">
                    ...
                    <input id="nama" type="text" name="nama" value="{{ old('nama', $kriterium->nama) }}" required autofocus
                ...
                <div class="mb-4">
                    ...
                    <input id="bobot" type="number" name="bobot" value="{{ old('bobot', $kriterium->bobot) }}" required step="0.01" min="0" max="1"
                ...
                <div class="mb-6">
                    ...
                    <select id="atribut" name="atribut" required
                        ...
                        <option value="benefit" {{ old('atribut', $kriterium->atribut) == 'benefit' ? 'selected' : '' }}>{{ __('Benefit') }}</option>
                        <option value="cost" {{ old('atribut', $kriterium->atribut) == 'cost' ? 'selected' : '' }}>{{ __('Cost') }}</option>
                    </select>
                    @error('atribut')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6 p-4 bg-gray-50 dark:bg-zinc-700 rounded-lg">
                    ...
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <span class="text-gray-900 dark:text-white">{{ $kriterium->created_at->format('M d, Y H:i') }}</span>
                        </div>
                        <div>
                            <span class="font-medium text-gray-600 dark:text-gray-400">Last Updated:</span>
                            <span class="text-gray-900 dark:text-white">{{ $kriterium->updated_at->format('M d, Y H:i') }}</span>
                        </div>
                    </div>
                </div>
