<x-layouts.app :title="__('Edit Alternatif') . ': ' . $alternatif->nama">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
                {{ __('Edit Alternatif') }}: {{ $alternatif->nama }}
            </h2>
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
            <form method="POST" action="{{ route('alternatif.update', $alternatif) }}">
                ...
                <div class="mb-4">
                    ...
                    <input id="nama" type="text" name="nama" value="{{ old('nama', $alternatif->nama) }}" required autofocus
                ...
                <div class="mb-6">
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-zinc-700 dark:text-white @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $alternatif->deskripsi) }}</textarea>
                ...
                <!-- Alternatif Info Display -->
                <div class="mb-6 p-4 bg-gray-50 dark:bg-zinc-700 rounded-lg">
                    <h3 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Alternatif Information</h3>
                    <div class="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            <span class="font-medium text-gray-600 dark:text-gray-400">ID:</span>
                            <span class="text-gray-900 dark:text-white">{{ $alternatif->id }}</span>
                        </div>
                        <div>
                            <span class="font-medium text-gray-600 dark:text-gray-400">Created:</span>
                            <span class="text-gray-900 dark:text-white">{{ $alternatif->created_at->format('M d, Y H:i') }}</span>
                        </div>
                        <div>
                            <span class="font-medium text-gray-600 dark:text-gray-400">Last Updated:</span>
                            <span class="text-gray-900 dark:text-white">{{ $alternatif->updated_at->format('M d, Y H:i') }}</span>
                        </div>
                    </div>
                </div>
